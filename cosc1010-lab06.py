...
letter_count = {letter : 0 for letter in random_string}
...
most_occured = 0
count_nums = list(letter_count.values())

for i in range(26):
    if count_nums[i] > most_occured:
        most_occured = count_nums[i]
for key, value in letter_count.items():
    if value == most_occured:
        most_occured = key
# Output which letter occurred the least
least_occurred = 1000
for i in range(26):
    if count_nums[i] < least_occurred:
        least_occurred = count_nums[i]
for key, value in letter_count .items():
    if value == least_occurred:
        least_occurred = key
...
print(f"The letter that occurred the most is {most_occured}")
print("*"*75)
# Output which letter occurred the least 
...
# Output what the percentage of the string each character is, again in alphabetical
for i in range(26):
    percentage = count_nums[i] / 2500
    print(f"{percentage * 100}% of the list was the letter {letters[i]}")